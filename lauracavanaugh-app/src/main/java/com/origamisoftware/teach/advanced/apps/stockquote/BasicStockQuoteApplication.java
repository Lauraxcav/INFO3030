...
public class BasicStockQuoteApplication {
    ...
    public String displayStockQuotes(StockQuery stockQuery) throws StockServiceException {
        ...
        List<StockQuote> stockQuotes =
                stockService.getQuote(stockQuery.getSymbol(), stockQuery.getFrom(), stockQuery.getUntil(), stockQuery.getIntervalEnum());
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 4) {
            exit(ProgramTerminationStatusEnum.ABNORMAL,
                    "Please supply 4 arguments a stock symbol, a start date (MM/DD/YYYY) and end date (MM/DD/YYYY), and an interval");
        }
        try {

            StockQuery stockQuery = new StockQuery(args[0], args[1], args[2], StockQuery.IntervalEnum.valueOf(args[3]));
